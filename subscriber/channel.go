...
func (c *ChannelConsumer) consume(workerID int) {
	...
	for {
		...
		case msg := <-c.messageCh:
			...
			if err != nil {
				c.monitor.Incr("labs_stream_errors", map[string]string{"task": c.svc.TaskName(), "service": c.groupID})
				c.logger.Warn("Moving on...")
				continue
			}

			c.monitor.Incr("labs_stream_count", map[string]string{"task": c.svc.TaskName(), "service": c.groupID})
		}
	}
}
...
func (c *ChannelConsumer) read() {
	...
	for {
		...
		default:
			...
			c.monitor.Gauge("labs_stream_lag", c.Stats().Lag, metricLabels)
			...
			c.monitor.Incr("labs_stream_events", metricLabels)
